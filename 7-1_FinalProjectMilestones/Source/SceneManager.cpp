 ...
 *  This method is used for setting the texture data
 *  associated with the passed in ID into the shader.
 *  Overload created by Cielo Neal.
 ***********************************************************/
void SceneManager::SetShaderTexture(
	std::string textureTag1, std::string textureTag2)
{
	if (NULL != m_pShaderManager)
	{
		m_pShaderManager->setIntValue(g_UseTextureName, true);
		...
		int textureID = -1;
		textureID = FindTextureSlot(textureTag1);
		m_pShaderManager->setSampler2DValue(g_TextureValueName, textureID);
		textureID = FindTextureSlot(textureTag2);
		m_pShaderManager->setSampler2DValue(g_TextureValueName, textureID);
	}
}
...
{
	OBJECT_MATERIAL diceMaterial;
	diceMaterial.ambientColor = glm::vec3(0.4f, 0.4f, 0.4f);
	diceMaterial.ambientStrength = 0.3f;
	diceMaterial.diffuseColor = glm::vec3(0.3f, 0.3f, 0.3f);
	diceMaterial.specularColor = glm::vec3(0.6f, 0.6f, 0.6f);
	diceMaterial.shininess = 85.0;
	diceMaterial.tag = "dice";

	m_objectMaterials.push_back(diceMaterial);
	...
	OBJECT_MATERIAL chestMaterial;
	chestMaterial.ambientColor = glm::vec3(0.19f, 0.19f, 0.19f);
	chestMaterial.ambientStrength = 0.3f;
	chestMaterial.diffuseColor = glm::vec3(0.5f, 0.5f, 0.5f);
	chestMaterial.specularColor = glm::vec3(0.2f, 0.2f, 0.2f);
	chestMaterial.shininess = 0.2;
	chestMaterial.tag = "chest";

	m_objectMaterials.push_back(chestMaterial);
}
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("lightSources[0].ambientColor", 0.05f, 0.01f, 0.01f);
...
void SceneManager::RenderScene()
{
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 1.05f, 3.0f);
	...
	//SetShaderColor(0.48, 0.43, 0.44, 1);
	SetShaderTexture("chest");
	SetShaderMaterial("chest");
	...
	/******************************************************************/
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.50f, 4.990f, 1.50f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(2.4980f, 2.0f, 3.0f);
	...
	/******************************************************************/
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.50f, 1.50f, 1.50f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.25f, 0.76f, 2.70f);
	...
	//SetShaderColor(0.65, 0, 0, 1);
	SetShaderTexture("dice");
	...
	/******************************************************************/
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(2.0f, 1.50f, 1.50f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(3.35f, 0.76f, 3.8f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(6.562f, 0.65f, 4.288f);
	...
	// set the XYZ rotation for the mesh
	XrotationDegrees = -68.0f;
	YrotationDegrees = -15.0f;
	ZrotationDegrees = 145.0f;

	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(6.0f, 0.225f, 5.0f);
	...
	/******************************************************************/
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.75f, 1.5f, 1.75f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-3.45f, 0.9f, 4.0f);
